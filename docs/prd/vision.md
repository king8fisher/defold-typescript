...
- **Impl**:
  ...
  - [types-api-coverage--go-module-fanout.md](../impl/types-api-coverage--go-module-fanout.md) — fifth slice: add the `go` (game object) namespace through the same `MODULE_MANIFEST` seam, exercising cross-module type refs (`vector3`, `quaternion`, `hash`, `url`) on a real, broadly-used Defold surface
